...
# also add it to `is_dirty` check
dirty = """    fn is_dirty(&self) -> bool {
        let a = &self.config;
        let b = &self.saved;
        self.edited_affinity() != b.cpu.default_affinity
            || a.probalance != b.probalance
            || a.monitor != b.monitor
            || a.hw_alerts != b.hw_alerts
            ...
            || self.autostart_enabled != crate::monitor::is_systemd_user_service_enabled()
    }"""
text = re.sub(r'    fn is_dirty\(&self\) -> bool \{.*?\}', dirty, text, flags=re.DOTALL)
